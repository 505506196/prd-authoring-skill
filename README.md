...
| 3 | 一审·用户自审 | 你亲自试用原型，提意见 |
| 4 | 详细PRD草稿 | 字段级/状态级/规则级需求（基于确认后的原型） |
...
- skill v0.3.0（七阶段：概要→原型→用户自审→详细PRD草稿→三审三校→汇总决策→最终交付；Iron Rule 1 支持 Element Plus 组件功能 + shekao-ui 视觉规范）
