...
# PRD 输出 — 概要PRD → 原型 → 详细PRD草稿 → 三审三校 → 汇总决策 → 最终交付

把需求做成可交付的 PRD。七个阶段流水线，每阶段：**读上一阶段落盘产物 → 与用户共创产出 → 过校验 → 停在闸口等确认 → 才进下一阶段**。

核心思路：先出**详细PRD草稿 + 原型**，经**三审三校**（用户自审 → 业务视角评审 → 研发需求评审），三审意见汇总交用户统一决策，用户确认后才输出**最终详细PRD + 最终原型**。
...
- 阶段5/6 跳过独立 subagent、自评自夸（见铁律2）
- 三审意见不汇总、AI 自行决定改不改（见铁律2）
- 最终PRD 无视三审另起炉灶（见铁律3）
...
流程完成 = 三审三校完成、用户决策后，阶段7输出最终交付物。
- 输出 `07-最终详细PRD.md` + 最终 `prototype/`，可直接进入研发排期
- 若三审发现 P0 阻塞项且用户认可需大改 → 阶段7先按裁决改原型/PRD，改不动的记入「暂缓」并说明
